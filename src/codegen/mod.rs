...
pub fn generate_all(manifest: &Manifest, output_dir: &str) -> Result<()> {
    ...
    println!("  [stub] Oblíbený codegen for '{}' — implementation pending", manifest.workload.name);
    Ok(())
}

pub fn build(manifest: &Manifest, _release: bool) -> Result<()> {
    println!("Building oblibeniser workload: {}", manifest.workload.name);
    Ok(())
}

pub fn run(manifest: &Manifest, _args: &[String]) -> Result<()> {
    println!("Running oblibeniser workload: {}", manifest.workload.name);
    Ok(())
}
